...
// src/lib/db.ts
var LEGACY_DEFAULT_SYSTEM_PROMPT = "You are a helpful voice assistant. Keep responses concise and conversational, suitable for being spoken aloud. Avoid markdown, bullet points, or any formatting - respond in plain spoken sentences only.";
async function ensureAiConfigSchema(db) {
  const info3 = await db.prepare(`PRAGMA table_info(ai_configs)`).all();
...
__name(ensureAiConfigSchema, "ensureAiConfigSchema");
async function upsertUser(db, { guest_id, email }) {
  let user = await db.prepare("SELECT * FROM users WHERE guest_id = ?").bind(guest_id).first();
  if (!user) {
    if (email) {
...
async function getOrCreateSession(db, userId) {
  let session = await db.prepare("SELECT * FROM sessions WHERE user_id = ? ORDER BY created_at ASC LIMIT 1").bind(userId).first();
  if (!session) {
    const id = crypto.randomUUID();
    await db.prepare("INSERT INTO sessions (id, user_id) VALUES (?, ?)").bind(id, userId).run();
    session = await db.prepare("SELECT * FROM sessions WHERE id = ?").bind(id).first();
  }
  return session;
}
__name(getOrCreateSession, "getOrCreateSession");
async function loadHistory(db, sessionId, limit = 20) {
...
async function insertMessage(db, sessionId, role, transcript) {
  const id = crypto.randomUUID();
  await db.prepare(
...
async function getUserByGuestId(db, guestId) {
  return db.prepare("SELECT * FROM users WHERE guest_id = ?").bind(guestId).first();
}
__name(getUserByGuestId, "getUserByGuestId");
async function getAiConfig(db, userId) {
  await ensureAiConfigSchema(db);
  const config2 = await db.prepare("SELECT * FROM ai_configs WHERE user_id = ?").bind(userId).first();
  if (!config2) {
    return {
      user_name: "",
      custom_instructions: ""
  ...
  const legacySystemPrompt = typeof config2.system_prompt === "string" ? config2.system_prompt : "";
  const legacyCustomInstructions = legacySystemPrompt && !legacySystemPrompt.startsWith("You are a helpful voice assistant.") ? legacySystemPrompt : "";
  return {
    user_name: config2.user_name ?? "",
    custom_instructions: config2.custom_instructions ?? legacyCustomInstructions,
    ai_name: config2.ai_name ?? "Assistant",
    system_prompt: config2.system_prompt ?? LEGACY_DEFAULT_SYSTEM_PROMPT
  };
}
__name(getAiConfig, "getAiConfig");
async function upsertAiConfig(db, userId, {
  user_name,
  custom_instructions
}) {
  await ensureAiConfigSchema(db);
  const existing = await db.prepare("SELECT id FROM ai_configs WHERE user_id = ?").bind(userId).first();
  if (existing) {
    await db.prepare(
      `UPDATE ai_configs
...
__name(upsertAiConfig, "upsertAiConfig");

// src/routes/session.ts
var app = new Hono2();
app.post("/", async (c) => {
  const { guest_id, email } = await c.req.json();
...
// src/lib/ai.ts
var BASE_SYSTEM_PROMPT = "You are a helpful voice assistant. Keep responses concise and conversational, suitable for being spoken aloud. Avoid markdown, bullet points, lists, code fences, or visual formatting. Respond in plain spoken sentences only.";
...
function generateMockWavBase64(durationSeconds = 1.5, frequency = 400, sampleRate = 16e3) {
  const numSamples = sampleRate * durationSeconds;
  const buffer = new ArrayBuffer(44 + numSamples * 2);
  const view = new DataView(buffer);
  const write = /* @__PURE__ */ __name((o, s) => s.split("").forEach((c, i) => view.setUint8(o + i, c.charCodeAt(0))), "write");
  write(0, "RIFF");
...
__name(generateMockWavBase64, "generateMockWavBase64");
async function runAIPipeline({
  ...
  try {
    ...
    if (msg.includes("Not logged in") || msg.includes("Unauthorized") || msg.includes("API token") || msg.includes("Binding AI needs to be run remotely")) {
      console.warn(
...
__name(runAIPipeline, "runAIPipeline");
...
`));
  ...
  c.executionCtx.waitUntil((async () => {
    try {
      ...
      send2("error", { message: `Pipeline failed: ${err.message || err}` });
    } finally {
      writer.close();
    }
  })());
...
app3.get("/", async (c) => {
  ...
  const user = await getUserByGuestId(c.env.DB, guestId);
  if (!user) return c.json({ error: "user not found" }, 404);
  const config2 = await getAiConfig(c.env.DB, user.id);
  console.log("GET /api/config result", { userId: user.id, config: config2 });
...
app3.post("/", async (c) => {
  ...
  if (!user) return c.json({ error: "user not found" }, 404);
  const config2 = await upsertAiConfig(c.env.DB, user.id, {
    user_name: (body.user_name ?? body.ai_name ?? "").trim(),
    custom_instructions: (body.custom_instructions ?? body.system_prompt ?? "").trim()
  });
  console.log("POST /api/config result", { userId: user.id, config: config2 });
  return c.json(config2);
});
...
app5.route("/api/session", app);
app5.route("/api/turn", app2);
app5.route("/api/config", app3);
app5.route("/api/history", app4);
...
// .wrangler/tmp/bundle-rX1OpN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rX1OpN/middleware-loader.entry.ts
