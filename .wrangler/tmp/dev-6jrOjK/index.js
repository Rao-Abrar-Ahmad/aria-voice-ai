...
async function ensureV2Schema(db) {
  ...
  const messageColumns = (messageInfo.results ?? []).map((row) => row.name);
  ...
  const configInfo = await db.prepare("PRAGMA table_info(ai_configs)").all();
  const configColumns = (configInfo.results ?? []).map((row) => row.name);
...
__name(ensureV2Schema, "ensureV2Schema");
...
async function getSessionBySessionId(db, sessionId) {
  return db.prepare("SELECT * FROM sessions WHERE id = ?").bind(sessionId).first();
}
__name(getSessionBySessionId, "getSessionBySessionId");
async function loadHistory(db, sessionId, limit = 50) {
...
async function insertMessage(db, sessionId, role, content) {
  await ensureV2Schema(db);
  const id = crypto.randomUUID();
  await db.prepare("INSERT INTO messages (id, session_id, role, content) VALUES (?, ?, ?, ?)").bind(id, sessionId, role, content).run();
  return { id, role, content };
}
__name(insertMessage, "insertMessage");
async function getAiConfig(db, userId) {
  await ensureV2Schema(db);
  const config2 = await db.prepare("SELECT ai_name, system_prompt FROM ai_configs WHERE user_id = ?").bind(userId).first();
  if (config2) return config2;
  const id = crypto.randomUUID();
  await db.prepare("INSERT INTO ai_configs (id, user_id, ai_name, system_prompt) VALUES (?, ?, ?, ?)").bind(id, userId, DEFAULT_AI_CONFIG.ai_name, DEFAULT_AI_CONFIG.system_prompt).run();
  return DEFAULT_AI_CONFIG;
}
__name(getAiConfig, "getAiConfig");
async function upsertAiConfig(db, userId, config2) {
  await ensureV2Schema(db);
  const normalized = {
    ai_name: config2.ai_name.trim() || DEFAULT_AI_CONFIG.ai_name,
    system_prompt: config2.system_prompt.trim() || DEFAULT_AI_CONFIG.system_prompt
  };
  const existing = await db.prepare("SELECT id FROM ai_configs WHERE user_id = ?").bind(userId).first();
  if (existing) {
    await db.prepare("UPDATE ai_configs SET ai_name = ?, system_prompt = ?, updated_at = datetime('now') WHERE user_id = ?").bind(normalized.ai_name, normalized.system_prompt, userId).run();
  } else {
    const id = crypto.randomUUID();
    await db.prepare("INSERT INTO ai_configs (id, user_id, ai_name, system_prompt) VALUES (?, ?, ?, ?)").bind(id, userId, normalized.ai_name, normalized.system_prompt).run();
  }
  return normalized;
}
...
app2.get("/", async (c) => {
  ...
  const config2 = await getAiConfig(c.env.DB, session.user_id);
...
app2.post("/", async (c) => {
  ...
  if (!session) return c.json({ error: "Session not found" }, 404);
  const body = await c.req.json();
  const config2 = await upsertAiConfig(c.env.DB, session.user_id, {
...
// src/lib/ai.ts
async function* runLLM(ai, history, config2) {
  const result = await ai.run("@cf/meta/llama-3.1-8b-instruct", {
    messages: [{ role: "system", content: config2.system_prompt }, ...history],
    max_tokens: 300,
    temperature: 0.7
  });
...
__name(runLLM, "runLLM");
async function runTTS(ai, text) {
...
var VoiceSession = class {
  ...
  getSessionId(request) {
    ...
    return cookie.split(";").map((part) => part.trim()).find((part) => part.startsWith("session_id="))?.split("=")[1];
  }
  async handleMessage(ws, sessionId, raw2) {
    let message;
    try {
      message = JSON.parse(String(raw2));
    } catch {
    ...
    try {
      ...
      this.send(ws, { type: "error", message: error3?.message ?? "Voice turn failed" });
      this.send(ws, { type: "status", state: "idle" });
    } finally {
      this.processing = false;
    }
  }
  async processTranscript(ws, sessionId, text) {
    ...
    const config2 = await getAiConfig(this.env.DB, session.user_id);
    const history = await loadHistory(this.env.DB, sessionId, 30);
    const llmInput = history;
    let assistantText = "";
    const generator = runLLM(this.env.AI, llmInput, config2);
    while (true) {
...
app4.route("/api/session", app);
app4.route("/api/config", app2);
app4.route("/api/history", app3);
app4.get("/ws", async (c) => {
...
// .wrangler/tmp/bundle-OdTDdg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OdTDdg/middleware-loader.entry.ts
