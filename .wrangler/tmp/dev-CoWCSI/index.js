...
// .wrangler/tmp/bundle-d7FHXs/strip-cf-connecting-ip-header.js
...
async function getAiConfig(db, userId) {
  ...
  if (!config2) {
    return {
      ai_name: "Assistant",
      system_prompt: "You are a helpful voice assistant. Keep responses concise and conversational, suitable for being spoken aloud. Avoid markdown, bullet points, or any formatting \u2014 respond in plain spoken sentences only."
    };
  }
  return config2;
}
__name(getAiConfig, "getAiConfig");
async function upsertAiConfig(db, userId, { ai_name, system_prompt }) {
...
app.post("/", async (c) => {
  ...
  if (!guest_id)
    return c.json({ error: "guest_id required" }, 400);
...
async function runAIPipeline({
  ...
  if (fallbackMode) {
    ...
    aiResponseText = `This is a mock response from ${aiConfig.ai_name}. I can see we have had ${history.length} messages. This is turn number ${turnCount}.`;
  } else {
    try {
...
app3.post("/", async (c) => {
  const { guest_id, ai_name, system_prompt } = await c.req.json();
  if (!guest_id)
    return c.json({ error: "guest_id required" }, 400);
  const user = await getUserByGuestId(c.env.DB, guest_id);
  if (!user)
    return c.json({ error: "user not found" }, 404);
  const config2 = await upsertAiConfig(c.env.DB, user.id, { ai_name, system_prompt });
  return c.json(config2);
});
...
// .wrangler/tmp/bundle-d7FHXs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-d7FHXs/middleware-loader.entry.ts
